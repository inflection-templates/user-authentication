...
import { UserAuthOptions, RequestType, ResourceOwnership, ActionScope } from './auth.types';
import { CurrentUser } from '../../domain.types/miscellaneous/current.user';
import { Middleware } from '../../domain.types/miscellaneous/middleware.types';
import ClientAppAuthMiddleware from '../client.app.auth/client.app.auth.middleware';

////////////////////////////////////////////////////////////////////////////////////////////////

export class UserAuthHandler {

    public static handle = (options: UserAuthOptions): Middleware[] => {
        ...
        // If the alternate authentication is in place.
    ...
    public static generateUserSessionToken = async (user: CurrentUser): Promise<string> => {
        var authenticator = Injector.Container.resolve(UserAuthenticator);
        return await authenticator.generateUserSessionToken(user);
    };

    public static generateRefreshToken = async (user: CurrentUser): Promise<string> => {
        var authenticator = Injector.Container.resolve(UserAuthenticator);
        return await authenticator.generateRefreshToken(user);
    };
    ...
    public static generatePasswordResetToken = async (userId: uuid, email: string, tenantId: uuid): Promise<string> => {
        var authenticator = Injector.Container.resolve(UserAuthenticator);
        return await authenticator.generatePasswordResetToken(userId, email, tenantId);
    };

    public static verifyPasswordResetToken = async (token: string): Promise<{ UserId: uuid, TenantId: uuid }|null> => {
        var authenticator = Injector.Container.resolve(UserAuthenticator);
        return await authenticator.verifyPasswordResetToken(token);
    };
...
export const auth = UserAuthHandler.handle;
