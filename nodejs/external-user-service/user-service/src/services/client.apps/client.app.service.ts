...
export class ClientAppService {
    ...
    createInternalClients = async (model: ClientAppCreateModel): Promise<ClientAppDto> => {
        const clientCode = await this.getClientCode(model.ClientName);
    ...
    getApiKey = async (model: ClientAppVerificationModel): Promise<ApiKeyDto> => {
        const client = await this.authenticateClientPassword(model);
        return await this._clientAppRepo.getApiKey(client.id);
    };

    renewApiKey = async (model: ClientAppVerificationModel): Promise<ApiKeyDto> => {
        const client = await this.authenticateClientPassword(model);
        const key = apikeyGenerator.default.create();
    ...
    authenticateClientPassword = async (model: ClientAppVerificationModel) => {
    ...
    update = async (id: string, clientDomainModel: ClientAppCreateModel): Promise<ClientAppDto> => {
    ...
    seedDefaultClients = async (): Promise<boolean> => {
        try {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (client == null) {
                    const model: ClientAppCreateModel = {
