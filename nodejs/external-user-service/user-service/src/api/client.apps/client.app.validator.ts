...
export class ClientAppValidator {
    ...
    static getDomainModel = async (body: any): Promise<ClientAppCreateModel> => {

        let clientModel: ClientAppCreateModel = null;
    ...
    static create = async (
        ...
        await body('ClientName').exists()
            ...
            .isLength({ min: 3 })
            .run(request);
        await body('PhoneCode').exists()
            .trim()
            .escape()
            .isLength({ min: 1 })
            .run(request);
        await body('PhoneNumber').exists()
            .trim()
            .escape()
            .isLength({ min: 10 })
            .run(request);
    ...
    static authenticateClientPassword = async (
        request: express.Request
    ): Promise<ClientAppVerificationModel> => {
    ...
    static getVerificationDomainModel = async (body: any, clientCode: string, password: string):
        ...
        let model: ClientAppVerificationModel = null;
        model = {
    ...
    static update = async (
        ...
        await body('ClientName').optional()
            ...
            .escape()
            .run(request);
        await body('PhoneNumber').optional()
            .trim()
            .escape()
            .isLength({ min: 10 })
            .run(request);
